...
const addons=[
  ...
  {id:'leatherseat',cat:'interior',name:'Leather Seat Cleaning & Conditioning',perUnit:12,minsPerUnit:12,unit:'seat',desc:'Gently removes dirt, body oils, and buildup, then conditions leather to help protect against drying and cracking.'},
  {id:'floormat',cat:'interior',name:'Carpet Floor Mat Deep Cleaning',perUnit:10,minsPerUnit:12,unit:'mat',desc:'Shampoos carpet floor mats and uses heated extraction to lift dirt, odors, embedded grime, and common stains.'},
...
document.getElementById('startOver').onclick=()=>{
  ...
  const ack=document.getElementById('inspectionAck'); if(ack) ack.checked=false;
  clearBookingState();
  renderVehicles();syncNavigationButtons();go(1)
};
...
function syncNavigationButtons(){
  ...
  if(vehicleNext) vehicleNext.disabled = !state.vehicle;
  if(typeNext) typeNext.disabled = !state.serviceType;
  if(packageNext) packageNext.disabled = !state.pkg;
  if(scheduleNext) scheduleNext.disabled = !(state.date && state.time);
}
...
/* Extra safety: any click on a vehicle card always selects it,
   enables Continue, saves state, and advances to Step 2. */
document.addEventListener('click', (event)=>{
  const vehicleCard = event.target.closest('#vehicleGrid .card.choice');
  if(vehicleCard){
    requestAnimationFrame(()=>{
      syncNavigationButtons();
      saveBookingState();
...
document.addEventListener('change', (event)=>{
  if(event.target && event.target.id === 'inspectionAck'){
